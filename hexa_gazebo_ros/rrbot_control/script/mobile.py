...
if __name__ == '__main__':
    ...
    Ty1 = Ty
    Tz1 = Tz
    # rottran = ["rotax", "rotay", "rotaz"]
    with open("historique.txt", "a+") as filout:
        filout.seek(0)
        content = filout.read()
        filout.write("|  ")
        filout.write(str(float(rotax)))
        filout.write("  |   ")
        filout.write(str(float(rotay)))
        filout.write("   |   ")
        filout.write(str(float(rotaz)))
        filout.write("   |   ")
        filout.write(str(float(translx)))
        filout.write("   |   ")
        filout.write(str(float(transly)))
        filout.write("   |   ")
        filout.write(str(float(translz)))
        filout.write("   |   ")
        filout.write(str(datetime.today()))
        filout.write("\n")
        filout.close()
    ...
    # x05=np.array([[-0.6], [0.555], [h2], [1]])

    # x6=np.array([[-0.6], [-0.11], [h], [1]])
    # x06=np.array([[-0.6], [-0.555], [h2], [1]])
    ...
    while not rospy.is_shutdown():
        ...
        t = rospy.get_time()
        f = 0.5
        thetax=Ax*math.sin(2*math.pi*f*t)
        thetay=Ay*math.sin(2*math.pi*f*t)
        thetaz=Az*math.sin(2*math.pi*f*t)

        Tx=Tx1*math.sin(2*math.pi*f*t)
        Ty=Ty1*math.sin(2*math.pi*f*t)
        Tz=Tz1*math.sin(2*math.pi*f*t)
